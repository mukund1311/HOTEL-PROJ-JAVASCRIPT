  ...
  async getProducts() {
    try {
      ...
      data = data.map((item) => {
        ...
        return { id, title, price, type, rating, id, url }
      });
      return data;
    } catch (error) {
      console.log(error);
    ...
    products.forEach((element) => {
      ...
      result += `
            ...
            <div class="img-container">
                ...
                <button data-id=${element.id} class="bag-btn">Add to cart</button>
                <p>${element.title}</p>
            </div>
            <div>
                <h4 class="light">TYPE: ${element.type}</h4>
      ...
      cartHtml += `<div class="cart-item">
                        ...
                        <div>   
                            <h4>${cartItem.title}</h4>
                             ...
                             <span class="remove-item" data-id=${cartItem.id}>remove
                         </div>
...
class Storage {
  ...
  static getLocalProduct(id) {
    return JSON.parse(localStorage.getItem("Products")).find((prod) => prod.id === id);
  }
